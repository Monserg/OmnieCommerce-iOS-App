...
class ConfirmQuestionView: CustomView {
    ...
    func createFromXIB() {
        UINib(nibName: String(describing: ConfirmQuestionView.self), bundle: Bundle(for: ConfirmQuestionView.self)).instantiate(withOwner: self, options: nil)
        addSubview(view)
        view.frame = frame
    }
    ...
    // MARK: - Actions
    @IBAction func handlerCancelButtonTap(_ sender: Any) {
        self.didHide()
