...
@IBDesignable class DottedBorderView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        if (viewStyle?.contains("Rectangle"))! {
            ...
            dottedLinePath.append(UIBezierPath(roundedRect: CGRect(x: self.frame.minX, y: self.frame.minY + lineWidth, width: self.frame.width - 2 * self.frame.minX, height: self.frame.maxY - 2 * lineWidth), cornerRadius: cornerRadius))
        } else {
        ...
        if (viewStyle?.hasSuffix("Color"))! {
            (Config.Constants.isAppThemesLight) ? UIColor.black.set() : UIColor.veryLightOrangeAlpha60.set()
        } else {
            (Config.Constants.isAppThemesLight) ? UIColor.black.set() : UIColor.lightGrayAlpha20.set()
        }
