...
class CalendarDayCellView: JTAppleDayCellView {
    ...
    func setTextColor(forState state: CellState) {
        ...
        case true:
            dayLabel.textColor = (Config.Constants.isAppThemesLight) ? UIColor.black : UIColor.veryLightGray

        default:
            if (state.dateBelongsTo == .thisMonth) {
                dayLabel.textColor = (Config.Constants.isAppThemesLight) ? UIColor.red : UIColor.veryLightGray
            } else {
                dayLabel.textColor = (Config.Constants.isAppThemesLight) ? UIColor.blue : UIColor.veryDarkGrayishBlue56
            }
        }
    }
